...
//swipl-ld -lallegro -lallegro_main -lallegro_font -o allegro_binding -shared allegro_binding.c
...
foreign_t al_wait_for_event_wrapper(term_t event_queue_in, term_t event_out){
  ...
  PL_unify_blob(ev_src, &event_source, sizeof(event_source), &event_source_ref_blob);
...
foreign_t al_create_display_wrapper(term_t w, term_t h, term_t display_out) {
...
foreign_t al_get_display_event_source_wrapper(term_t display_in, term_t source_out) {
  ...
  PL_unify_blob(source_out, &event_source, sizeof(event_source), &event_source_ref_blob);
  PL_succeed;
}
